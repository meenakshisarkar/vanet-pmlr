...
class VANET(object):
    ...
    def forward_model(self, vel_in, acc_in, xt, cell):
        ...
        for t in xrange(self.timesteps - 1):
            h_vel_out, vel_state, vel_res_in = self.vel_enc(vel_in[:, :, :, t, :], vel_state, reuse=reuse_vel)
            if t<=self.timesteps-2:
                h_acc_out, acc_state, acc_res_in = self.acc_enc(acc_in[:, :, :, t, :], acc_state, reuse=reuse_acc)
            reuse_vel = True
            reuse_acc = True
    ...
    def vel_enc(self, vel_in, vel_state, vel_LSTM, reuse):
        ...
        h1_state, state = vel_LSTM(pool3, state, scope='vel_lstm1', reuse=reuse)
        h2_state, state = vel_LSTM(h1_state, state, scope='vel_lstm2', reuse=reuse)
        h_vel_out, state = vel_LSTM(h2_state, state, scope='vel_lstm3', reuse=reuse)
        return h_vel_out, vel_state, vel_res_in

    def acc_end(self, acc_in, acc_LSTM, reuse):
        acc_res_in = []
        ...
        pool3 = MaxPooling(conv3, [2, 2])
        h1_state, state = acc_LSTM(pool3, state, scope='acc_lstm1', reuse=reuse)
        h2_state, state = acc_LSTM(h1_state, state, scope='acc_lstm2', reuse=reuse)
        h_acc_out, state = acc_LSTM(h2_state, state, scope='acc_lstm3', reuse=reuse)
        return h_acc_out, state, acc_res_in
