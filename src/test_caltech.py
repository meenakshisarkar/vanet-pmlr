...
def main(lr, batch_size, alpha, beta, image_h, image_w, vid_type, K,
    ...
    data_path = "../data/CalTech_ped/processed/test"
    test_dirs=[]
    dirs_len=[]
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False, gpu_options=gpu_options)) as sess:                                     #if gpus else None
        ...
        if success_load_model[0]:
            ...
            print(" [!] Load failed...")
            return
        quant_dir = "../results/quantitative/CalTech/"+prefix_test+"/"+model_number+"/"
        ...
        for d, l in data_dict.items():
            ...
            # d = test_dirs[i]
            seq_batch, diff_batch, accel_batch = load_caltech_data(d,l,(image_h, image_w), K, T)
            ...
            savedir = os.path.join('../results/images/CalTech/'+prefix_test+'/'+model_number,'/'.join(d.split('/')[-4:]))
            print (savedir )
