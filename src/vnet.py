...
class VNET(object):
    ...
    def forward_model(self, vel_in, xt, vel_LSTM):
        ...
        for t in range(self.F):
            if t==0:
                ...
                res_conv= self.res_conv_layer(con_res_in, vel_res_in,name="res_conv_layer", reuse= False)
                ...
                cont_conv = self.conv_layer(h_con_state, h_vel_out, name="conv_layer",reuse= True)
                res_conv= self.res_conv_layer(con_res_in, vel_res_in, name="res_conv_layer", reuse= True)
    ...
    def res_conv_layer(self, con_res_in, vel_res_in, name, reuse):
        with tf.variable_scope(name, reuse):
            "$Need to be updated$"
            res_conv_out=[]
            #no_layers= len(con_res_in)
            res_motion_in1= tf.concat([con_res_in[0], vel_res_in[0]], axis= 3)
            res_cont_conv1_1= relu(conv2d(res_motion_in1, output_dim= vel_res_in[0].shape[-1], k_h=3, k_w=3,
                                            d_h=1, d_w=1, name= "res_conv_layer1_1", reuse=reuse))
            res_cont_conv1_2=relu(conv2d(res_cont_conv1_1,output_dim= vel_res_in[0].shape[-1], k_h=3, k_w=3,
                                            d_h=1, d_w=1, name= "res_conv_layer1_2", reuse=reuse))
            res_conv_out.append(res_cont_conv1_2)

            res_motion_in2= tf.concat([con_res_in[1], vel_res_in[1]], axis= 3 )
            res_cont_conv2_1= relu(conv2d(res_motion_in2, output_dim= vel_res_in[1].shape[-1], k_h=3, k_w=3,
                                            d_h=1, d_w=1, name= "res_conv_layer2_1", reuse=reuse))
            res_cont_conv2_2=relu(conv2d(res_cont_conv2_1,output_dim= vel_res_in[1].shape[-1], k_h=3, k_w=3,
                                            d_h=1, d_w=1, name= "res_conv_layer2_2", reuse=reuse ))
            res_conv_out.append(res_cont_conv2_2)

            res_motion_in3 = tf.concat([con_res_in[2], vel_res_in[2]], axis= 3 )
            res_cont_conv3_1= relu(conv2d(res_motion_in3, output_dim= vel_res_in[2].shape[-1], k_h=3, k_w=3,
                                            d_h=1, d_w=1, name= "res_conv_layer3_1", reuse=reuse))
            res_cont_conv3_2=relu(conv2d(res_cont_conv3_1,output_dim= vel_res_in[2].shape[-1], k_h=3, k_w=3,
